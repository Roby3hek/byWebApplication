<?php
if (isset($_POST['fio']) && isset($_POST['adress'])&& isset($_POST['number'])&& isset($_POST['promo'])){
    // Переменные с формы
    $fio = $_POST['fio'];
    $adress = $_POST['adress'];
    $number = $_POST['number'];
    ...
    try {
        ...
        if (empty($number)){
            ...
            $data = array('title_id'=>"1",'fio' => $fio, 'adress' => $adress, 'number' => $number ); 
    ...
    if ($result && $promo == 'reg10el') {
    	echo "Заказ успешно оформлен, стоимость <br> с учетом промокода составляет 4500 рублей, <br> ожидайте звонка менеджера";
    } 
    else if ($result){
        echo "Заказ успешно оформлен, стоимость <br> составляет 5000 рублей, ожидайте <br> звонка менеджера";
    }
}?>
<!DOCTYPE html>
<html lang="ru">
...
<title>Форма заказа электрика</title>
...
<body>
    ...
    <div>
        <p  class="PROMO" >Введите промокод(если есть)</p>
        <input name="promo" type="text"  autocorrect=off autocapitalize = words />
    </div>
    <div class ="but">
        <input type="submit" value="Оплатить наличными"/><br><br>
